...
class VerifyUserOtpScreen extends StatelessWidget {
  static const String name = 'verify_otp_user_screen';
  const VerifyUserOtpScreen({
    super.key,
    required this.email,
  });
  final String email;
...
class UserFormOtp extends ConsumerWidget {
  const UserFormOtp({
    super.key,
    required this.email,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future.microtask(() {
      ref.read(otpUserFormProvider.notifier).onEmailChange(email);
    });
    ref.listen(
      authProvider,
      (previous, next) {
    ...
    final otpNotifier = ref.read(otpUserFormProvider.notifier);
    return Padding(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: CustomFilledButton(
              text: 'VERIFICAR',
              onPressed: ref.read(otpUserFormProvider.notifier).onFormSubmitOtp,
            ),
          ),
          const SizedBox(height: 20),
          TextButton(
